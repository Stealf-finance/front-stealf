 ...
 */
export interface MWAWalletBridge {
  type: string;
...
export function createSeedVaultWallet(
  ...
  return {
    ...
    async signAndSendTransaction(
      ...
      const { transact } = require("@solana-mobile/mobile-wallet-adapter-protocol-web3js");
      // Use signTransactions + manual send (more reliable than signAndSendTransactions)
      const signedBytes = await transact(async (wallet: Web3MobileWallet) => {
        await mwaAuthorize(wallet, authToken);

        const tx = Transaction.from(serializedTx);
        const signedTxs = await wallet.signTransactions({ transactions: [tx] });
        const signedTx = signedTxs[0] as Transaction;
        return new Uint8Array(signedTx.serialize({ requireAllSignatures: false, verifySignatures: false }));
      });
