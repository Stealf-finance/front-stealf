...
export default function LoginScreen({ onBack, onSuccess, onSwitchToRegister }: LoginScreenProps) {
  const { login } = useAuth();
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (data.details && Array.isArray(data.details) && data.details.length > 0) {
          ...
          if (errorDetail.code === 'grid_account_not_found') {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      const url = `${API_URL}/grid/auth/verify`;
