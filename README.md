...
### Passkey Error "No Credentials were returned"

1. Vérifier que le fichier AASA est accessible: `curl https://stealf.xyz/.well-known/apple-app-site-association`
2. Vérifier que l'app est installée sur appareil physique (pas simulateur)
3. Clean build + réinstaller l'app
4. Supprimer l'app et rebuilder complètement

### Backend non accessible depuis l'iPhone

1. Vérifier que l'iPhone et le Mac sont sur le même réseau WiFi
2. Utiliser l'IP locale du Mac dans `.env` (pas `localhost`)
3. Vérifier que le backend écoute sur `0.0.0.0:3000` et pas `127.0.0.1:3000`